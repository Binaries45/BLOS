...
pub fn init(fb_info: FbInfo) @This() {
    fb_ptr = fb_info.ptr;
...
pub fn clear(t: *@This(), c: Color) void {
    t.cx = 0;
    t.cy = 0;
...
pub fn putChar(t: *@This(), c: u8) void {
    if(c == '\n') {
        t.cx = 0;
        t.cy += header.height;
        return;
    ...
    const g_data = font_bytes[g_start..g_start + header.bytesperglyph];
...
pub fn print(t: *@This(), s: []const u8) void {
    for (s) |c| t.putChar(c);
}
